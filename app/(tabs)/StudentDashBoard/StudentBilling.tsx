...
const StudentBilling = () => {
  ...
  const [studentName, setStudentName] = useState(routeStudentName || 'Student');
  ...
  useEffect(() => {
    ...
    const verifyUserRole = async () => {
      try {
        ...
        if (userRole !== "student") {
          console.log("❌ Non-student trying to access student billing, redirecting...");
        ...
        if (auth?.email) {
          ...
          if (!routeStudentName || !routeProfileImage) {
            ...
            if (res.ok) {
              ...
              if (!routeStudentName) setStudentName(data.name || "Student");
    ...
    const loadBillingData = async () => {
      try {
        ...
        // Fetch student billing data from API
        const response = await fetch(`${BASE_URL}/api/billing/student-invoices`, {
          method: 'GET',
          headers,
        });
  ...
  const handleSidebarSelect = (item: string) => {
    ...
    // Handle navigation - only student routes allowed
    switch (item) {
      case 'Home':
      ...
      case 'Profile':
        ...
        break;
      case 'Share':
        router.push({ pathname: "/(tabs)/StudentDashBoard/Share", params: { userEmail } } as any);
        break;
      case 'Billing':
  ...
  // Desktop Layout - Student Version
  return (
