    ...
    <View style={styles.container}>
      <View style={styles.topContainer}>
        <View style={styles.titleContent}>
          <Text style={styles.title}>Confirm Your Class :</Text>
          <TouchableOpacity onPress={() => router.back()}>
            {/* <DangerousIcon color="#FFF" /> */}
              <Ionicons name="close" size={wp("10%")} color="#FFF" />
            </TouchableOpacity>
        </View>
        <View style={styles.lableContainer}>
          <View style={styles.label}><BulbIcon color="#FFF" /><Text style={styles.text}>Study now, pay later</Text></View>
          <View style={styles.label}><BulbIcon color="#FFF" /><Text style={styles.text}>Best Teachers from anywhere</Text></View>
          <View style={styles.label}><BulbIcon color="#FFF" /><Text style={styles.text}>Get the best classes</Text></View>
        </View>
      </View>
              ...
              <View style={styles.checkboxContainer}>
                ...
                <View style={styles.leftSection}>
                  <View style={styles.dayTimeContainer}>
                    ...
                    <View style={styles.timeContainer}>
                      <Text style={styles.timing}>{t.timeFrom}</Text>
                      <Text style={styles.timing}>{t.timeTo}</Text>
                    </View>
                  </View>
                </View>
                ...
                <View style={styles.rightSection}>
                  <View style={styles.subjectContainer}>
                    {t.skill ? (
                      <Text style={styles.subject}>{t.skill}</Text>
                    ) : (
                      <>
                        <Text style={styles.subject}>{t.subject}</Text>
                        <Text style={styles.className}>{t.class}</Text>
                      </>
                    )}
                  </View>
                    <CustomCheckbox 
                      ...
                      size={wp("5.66%") * 1.5} // Match your scaling
                    />
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: wp("8.266%"), fontWeight: "700", lineHeight: hp("6.325%"), color: "#fff", width: width * 0.7 },
  label: { flexDirection: "row", alignItems: "center", justifyContent: "center", alignContent: "flex-start", gap: wp("2.933%") },
  text: { fontSize: wp("4%"), lineHeight: hp("2.96%"), color: "#fff", fontWeight: "700", marginTop: hp("1.08%") },
  ...
  leftSection: { width: "33%", paddingRight: wp("2%") },
  dayTimeContainer: { flex: 1, justifyContent: "center" },
  dayRow: { flexDirection: "row", alignItems: "center", marginBottom: hp("0.8%") },
  chevronButton: { marginRight: wp("1%") },
  day: { fontWeight: "600", fontSize: wp("2.93%"), lineHeight: hp("1.884%"), color: "#000000", flex: 1 },
  timeContainer: { marginLeft: wp("4%") },
  timing: { fontSize: wp("3.2%"), color: "#000000", lineHeight: hp("2.2%") },
  separator: { width: wp("0.44%"), backgroundColor: "#ccc", height: "130%", marginHorizontal: wp("2%") },
  rightSection: { flex: 1, flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingLeft: wp("2%") },
  subjectContainer: { flex: 1 },
  subject: { fontWeight: "600", fontSize: wp("3.733%"), marginBottom: hp("0.5%") },
  className: { fontSize: wp("3.2%"), color: "#555", lineHeight: hp("2.15%") },
  ...
  dayText: { fontSize: wp("3%"), color: "#000", fontWeight: "500" },
