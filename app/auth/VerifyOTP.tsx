      ...
      <View style={webStyles.container}>
        ...
        <View style={webStyles.rightColumn}>
          <View style={webStyles.content}>
            ...
            <Image source={require("../../assets/image/otp.png")} style={webStyles.image} />

            <Text style={webStyles.title}>Enter OTP</Text>
            ...
            <View style={webStyles.otpContainer}>
              {otp.map((digit, index) => (
                <View key={index} style={webStyles.otpInputWrapper}>
                  <TextInput
                    ref={(ref) => { inputRefs.current[index] = ref; }}
                    style={[webStyles.otpInput, digit ? webStyles.otpInputFilled : webStyles.otpInputEmpty]}
                    value={digit}
                    ...
                    caretHidden={true}
                  />
                  {!digit && <Text style={webStyles.dashPlaceholder}>-</Text>}
                </View>
              ))}
            </View>

            <TouchableOpacity 
              style={[webStyles.button, (!isOtpComplete || loading) && webStyles.buttonDisabled]} 
              onPress={handleVerifyOTP} 
              disabled={!isOtpComplete || loading}
            >
              {loading ? <ActivityIndicator color="#fff" /> : <Text style={webStyles.buttonText}>Submit</Text>}
            </TouchableOpacity>

            {resendVisible ? (
              <TouchableOpacity onPress={handleResendOTP} disabled={loading}>
                <Text style={webStyles.resendText}>Didn't get OTP? <Text style={webStyles.resendLink}>Resend</Text></Text>
              </TouchableOpacity>
            ) : (
              <Text style={webStyles.timerText}>
                Didn't get OTP? Resend in <Text style={webStyles.timerHighlight}>{String(Math.floor(timer / 60)).padStart(2, "0")}:{String(timer % 60).padStart(2, "0")}</Text>
              </Text>
            )}
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: wp("6.4%"), fontFamily: "WorkSans-Bold", marginBottom: hp("1.5%"), lineHeight: hp("4%"), color: "#0c0c0c", textAlign: "center" },
  subtitle: { fontSize: wp("2.6%"), fontFamily: "WorkSans-Regular", textAlign: "center", color: "#656565", marginBottom: hp("3%"), lineHeight: hp("2.5%") },
  ...
  otpInput: { width: "100%", height: "100%", fontSize: wp("9%"), fontFamily: "WorkSans-SemiBold", textAlign: "center", backgroundColor: "transparent", zIndex: 2 },
  otpInputEmpty: { color: "transparent" },
  otpInputFilled: { color: "#000" },
  dashPlaceholder: { position: "absolute", top: 0, left: 0, right: 0, bottom: 0, fontSize: wp("12%"), fontFamily: "WorkSans-Regular", textAlign: "center", color: "#504d4dff", lineHeight: wp("12.5%") },
  ...
  buttonText: { color: "#fff", fontSize: wp("4.2%"), fontFamily: "WorkSans-SemiBold" },
  timerText: { fontSize: wp("3.5%"), fontFamily: "WorkSans-Regular", color: "#666666", marginTop: hp("2%"), textAlign: "center" },
  timerHighlight: { color: "#2288ff", fontFamily: "WorkSans-Regular" },
  resendText: { fontSize: wp("3.5%"), fontFamily: "WorkSans-Regular", color: "#666666", marginTop: hp("2%") },
  resendLink: { color: "#5f5fff", fontFamily: "WorkSans-SemiBold" },
});
